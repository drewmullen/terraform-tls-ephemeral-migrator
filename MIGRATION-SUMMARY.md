...
**Files Modified:**
- `main.tf` - migrated 1 resource, 1 consumer resource
...
- Split into `legacy` (count) and `ephemeral` (count) resources
- Added `moved` block: `tls_private_key.legacy` → `tls_private_key.legacy[0]`
- Updated 1 consumer with separate attribute paths

**Consumers updated:**
- `vault_kv_secret_v2.legacy` - now uses `data_json` vs `data_json_wo`
...
- Added `moved` block: `vault_kv_secret_v2.legacy` → `vault_kv_secret_v2.legacy[0]`
...
4. Run `terraform apply` - existing secrets regenerated
5. Old secrets removed from state
...
3. Tag new minor version (e.g., v1.1.0)
